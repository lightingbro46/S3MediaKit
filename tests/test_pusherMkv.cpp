...
int domain(const string &file, const string &url) {
    ...
    // Load the MediaSource object from the mkv file
    auto tuple = MediaTuple {DEFAULT_VHOST, "live", "stream", ""};
    auto reader = std::make_shared<MKVReader>(tuple, file);
    // Start loading the mkv, set ref_self to false, so that the reader object can be set to nullptr to cancel, file_repeat can be set to empty, so that the file is read and the push stream is stopped
    reader->startReadMKV(100, false, true);
    ...
    // src is used up, can be set to empty directly, to prevent the main function from holding it (MKVReader holds it)
    src = nullptr;
...
int main(int argc, char *argv[]) {
    // You can use the mkv file generated by test_server
    // File uses absolute path, push URL supports rtsp and rtmp
    // return domain("/Users/xiongziliang/Downloads/mkv/Quantum.mkv", "rtsp://127.0.0.1/live/rtsp_push");

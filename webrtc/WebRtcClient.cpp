...
void WebRTCUrl::parse(const string &strUrl, bool isPlayer) {
    ...
    if (split_vec.size() > 0) {
        ...
        if (_vhost == "localhost" || isIP(_vhost.data())) {
            // If accessing localhost or an IP, set the default virtual host
    ...
    if (!enableVhost || _vhost.empty()) {
        // If virtual host is disabled or empty, set the virtual host to default
        _vhost = DEFAULT_VHOST;
    }
...
void WebRtcClient::onNegotiateFinish() {
    ...
    if (WebRtcTransport::SignalingProtocols::WEBSOCKET == _url._signaling_protocols) {
        // P2P mode requires gathering candidates
        gatheringCandidate(_peer->getIceServer());
    } else if (WebRtcTransport::SignalingProtocols::WHEP_WHIP == _url._signaling_protocols) {
        // SFU mode does not have IP connectivity issues, candidates are included in the answer, directly perform connectivityCheck
        connectivityCheck();
    }
}
...
void WebRtcClient::doNegotiateWebsocket() {
    DebugL;
#if 0
    //TODO: Currently each call uses a separate peer_connection, no reuse
    ...
    // Unregistered, first add the registration process, and unregister after this playback ends

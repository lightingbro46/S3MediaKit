#ifndef S3MEDIAKIT_WEBRTC_STUN_PACKET_HPP
#define S3MEDIAKIT_WEBRTC_STUN_PACKET_HPP
...
public:
    ...
    enum class Code : uint16_t {
        Invalid                     = 0,   //
        TryAlternate                = 300, //Try an alternate server
        BadRequest                  = 400,
        Unauthorized                = 401,
        Forbidden                   = 403, //prohibit
        RequestTimedOut             = 408, //Request timeout (client thinks this transaction has failed)
        UnknownAttribute            = 420,
        AllocationMismatch          = 438,
        StaleNonce                  = 438, //NONCE is no longer valid, the client should try again using the NONCE in the response
        AddressFamilyNotSupported   = 440, //Unsupported protocol family
        WrongCredentials            = 441, //Bad credentials
        UnsupportedTransportAddress = 442, //Unsupported transport address
        AllocationQuotaReached      = 486, //allocation reaches the upper limit, the client should wait at least one minute before trying to create again.
        RoleConflict                = 487, //role conflict
        ServerError                 = 500, //Temporary server error, client should try again
        InsuficientCapacity         = 508, //Insufficient capacity, no more relay transport addresses available
    };
...
private:
    // Length less than 128 characters
    std::string _realm;
};
...
protected:
    ...
    size_t                        _message_integrity_data_len = 0; //The field before the MESSAGE_INTEGRITY attribute

...
// Check if the ICE transport policy allows this candidate pair
static bool checkIceTransportPolicy(const IceAgent::CandidatePair& pair_info, const IceTransport::Pair::Ptr& pair) {
    ...
    // Prefer using the new unified configuration parameter
    switch (static_cast<IceTransportPolicy>(ice_transport_policy)) {
        case IceTransportPolicy::kRelayOnly:
            // Relay only: require local or remote to be relay type
        ...
        case IceTransportPolicy::kP2POnly:
            // P2P only: require local and remote to not be relay type
            if (pair_info._local_candidate._type == CandidateInfo::AddressType::RELAY ||
                pair_info._remote_candidate._type == CandidateInfo::AddressType::RELAY) {
...
void IceServerInfo::parse(const std::string &url_in) {
    ...
    // Parse ? parameters after parsing username and password to prevent ? in password from being treated as parameter delimiter
    auto pos = url.find("?");
    if (pos != string::npos) {
...
void IceTransport::processResponse(const StunPacket::Ptr& packet, const Pair::Ptr& pair) {
    ...
    // Clear request information immediately after receiving the response
...
bool IceTransport::processChannelData(const uint8_t* data, size_t len, const Pair::Ptr& pair) {
    ...
    // Check if it is a ChannelData message
    // The first two bytes of the ChannelData message are the channel number, the range is 0x4000-0x7FFF
    if (header.channel_number < 0x4000 || header.channel_number > 0x7FFF) {
        // WarnL << "Invalid channel number: " << header.channel_number;
        return false;
    }

    // This is a ChannelData message; check if the data length is valid
...
void IceTransport::sendChannelData(uint16_t channel_number, const Buffer::Ptr& buffer, const Pair::Ptr& pair) {
    ...
    // ChannelData is not a STUN message and needs to be implemented separately
    // ChannelData format: 2 bytes Channel Number + 2 bytes data length + data content
    auto data_len = buffer->size();
    size_t total_len = 4 + data_len;
    // Allocate buffer: 4 bytes header + data length
    auto channel_data = toolkit::BufferRaw::create(total_len);
    auto header = reinterpret_cast<ChannelDataHeader *>(channel_data->data());
    // Set Channel Number (first two bytes, network byte order)
    header->channel_number = htons(channel_number);
    // Set data length (middle two bytes, network byte order)
...
bool IceTransport::hasPermission(const sockaddr_storage& addr) {
    ...
    // Permission is valid for 5 minutes
    ...
    std::shared_ptr<uint16_t> getSinglePort() {
        ...
        std::shared_ptr<uint16_t> ret(new uint16_t(pos), [weak_self, pos](uint16_t *ptr) {
            ...
            if (pos >= strong_self->_min_port && pos < strong_self->_max_port) {
                ...
                // Port range has been modified, this port is no longer within the range, do not recycle port number
            }
        });
...
private:
    void setRange(uint16_t min_port, uint16_t max_port) {
        ...
        lock_guard<recursive_mutex> lck(_pool_mtx);
        // Port range has not changed, no need to process
        ...
        // Modification: use port values directly, no longer divide by 2
        uint16_t start_pos = min_port;
        uint16_t end_pos = max_port;
        std::mt19937 rng(std::random_device {}());

        // New specified port range does not intersect with the original port range, clear and re-add directly
        if (max_port <= _min_port || min_port >= _max_port) {
            _port_pool.clear();
        } else {
 
            // There is an intersection, first keep the ports in the intersection range that have not been allocated yet
            ...
            if (_min_port <= min_port && _max_port < max_port) {
                ...
                while (start_pos < end_pos) {
                    // Random port sorting to prevent duplicate assigned ports after restarting
...
//Register port management listening configuration reload
...
void IceServer::handleCreatePermissionRequest(const StunPacket::Ptr& packet, const Pair::Ptr& pair) {
    ...
    // Check if the XOR-PEER-ADDRESS attribute exists
...
void IceServer::handleChannelBindRequest(const StunPacket::Ptr& packet, const Pair::Ptr& pair) {
    ...
    // Verify that the channel number is within the valid range (0x4000-0x7FFF)
    ...
    // Check whether there is permission for the corresponding peer address
    ...
    // Add or update channel bindings
...
void IceServer::handleChannelData(uint16_t channel_number, const char* data, size_t len, const Pair::Ptr& pair) {
    ...
    // Find the target address corresponding to the channel number
    ...
    // Create a new buffer for forwarding
...
StunPacket::Authentication IceServer::checkRequestAuthentication(const StunPacket::Ptr& packet, const Pair::Ptr& pair) {
    ...
    //ICE SERVER does not verify BINDGING request
...
SocketHelper::Ptr IceServer::allocateRelayed(const Pair::Ptr& pair) {
    ...
    //If an external network card is specified, use the IP of the first external network card.
    if (!interfaces.empty()) {
    ...
    getPoller()->async([=]() {
        ...
        if (it == _relayed_pairs.end()) {
#if 0
            // Not the forwarding of the current object, hand it over to other objects for forwarding
            ...
            if (getIdentifier() == forword_session->getIdentifier()) {
                //The session found is the current session, ignored
                return;
            }
...
: IceTransport(listener, std::move(ufrag), std::move(password), std::move(poller)), _implementation(implementation) ,_role(role) {
    ...
    // Create a timer to check permissions and channel bindings every minute
...
void IceAgent::gatheringCandidate(const CandidateTuple::Ptr& candidate_tuple, bool gathering_rflx, bool gathering_relay) {
    ...
    for (auto obj : interfaces) {
        ...
        try {
            ...
            if (gathering_relay) {
                //TODO: Optimize relay_socket reuse of host socket, current SocketCandidateManager data structure does not support it
...
void IceAgent::tryTriggerredCheck(const Pair::Ptr& pair) {
    DebugL;
    //FIXME Not implemented for now, because the current implementation basically initiates a check upon receiving a candidate
}
...
void IceAgent::handleConnectivityCheckResponse(const StunPacket::Ptr& packet, const Pair::Ptr& pair, CandidateTuple& candidate) {
    ...
    if (!pair->_relayed_addr) {
        //Relay messages do not add PRFLX candidaite
...
void IceAgent::handleCreatePermissionResponse(const StunPacket::Ptr& packet, const Pair::Ptr& pair, const sockaddr_storage& peer_addr) {
    ...
    static uint16_t next_channel = 0x4000; // Valid range is 0x4000-0x7FFF
    uint16_t channel_number = next_channel++;
    if (next_channel > 0x7FFF) {
        next_channel = 0x4000; // Loop use channel number
    }
...
void IceAgent::handleDataIndication(const StunPacket::Ptr& packet, const Pair::Ptr& pair) {
    ...
    // Notify upper layer of received data
...
void IceAgent::onGatheringCandidate(const Pair::Ptr& pair, CandidateInfo& candidate) {
    ...
    // Use _socket_candidate_manager instead of _local_candidates for 5-tuple duplicate checking
    ...
    //If it is REALY, all current PEER Candidates perform CreatePermission
...
void IceAgent::onConnected(const IceTransport::Pair::Ptr& pair) {
    ...
    // Check whether the ConnectivityCheck pair is in the checklist, if so, add it to the valid list
    for (auto &candidate_pair : _check_list) {
        ...
        //Even the new Peer reflection address has been added to _checklist
        //So a match will definitely be found in _checklist
        ...
        // Directly add the candidate pair to the valid_list
...
void IceAgent::onCompleted(const IceTransport::Pair::Ptr& pair) {
    ...
    if (getImplementation() == Implementation::Full) {
        ...
        if (!found_in_valid_list) {
            ...
            // The nominated candidate was not found in _valid_list. Record it first.
            _nominated_pair = pair;
        }
    } else {
        //Lite mode, no candidate verification logic
        found_in_valid_list = true;
    }
...
void IceAgent::refreshPermissions() {
    ...
    // Traverse all permissions and delete expired permissions
    ...
    for (auto& permission : _permissions) {
        if (now - permission.second > 4 * 60 * 1000) {
            ...
            for (auto& socket : _socket_candidate_manager._relay_sockets) {
                ...
                break; // Only need to use one local candidate to send the request
            }
        }
    }
}
...
void IceAgent::refreshChannelBindings() {
    ...
    // Traverse all channel bindings and delete expired bindings
    for (auto it = _channel_binding_times.begin(); it != _channel_binding_times.end();) {
        if (now - it->second > 10 * 60 * 1000) { // Channel binding is valid for 10 minutes
    ...
    // For channel bindings that are about to expire (e.g. 2 minutes left to expire), refresh them
        ...
        if (now - binding_time.second > 8 * 60 * 1000) {
            ...
            if (it != _channel_bindings.end()) {
                ...
                for (auto& socket : _socket_candidate_manager._relay_sockets) {
                    ...
                    break; // Just send the request using a local candidate
                }
            }
        }
    }
...
CandidateInfo IceAgent::getLocalCandidateInfo(const Pair::Ptr& pair) {
    // Find the corresponding local candidate information from socket_candidate_manager
    for (const auto& socket_candidates : _socket_candidate_manager.socket_to_candidates) {
        if (socket_candidates.first == pair->_socket) {
            // Find the candidate list corresponding to the socket and select the first one (host type) as the default
    ...
    for (auto it = _response_handlers.begin(); it != _response_handlers.end();) {
        ...
        if (now >= req_info._next_timeout) {
            if (req_info._retry_count >= max_retry) {
                // If the maximum number of retransmissions is exceeded, the request is abandoned and cleared.
...
void IceTransport::retransmitRequest(const std::string& transaction_id, RequestInfo& req_info) {
    ...
    // RTO doubled (exponential backoff)
    req_info._rto *= 2;

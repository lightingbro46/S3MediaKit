...
static std::string openssl_MD5(const void *data, size_t data_len) {
    ...
    unsigned int out_len;
#if defined(OPENSSL_VERSION_NUMBER) && (OPENSSL_VERSION_NUMBER > 0x10100000L)
    // Openssl 1.1.0 new API, old version API is invalid
...
StunPacket::Authentication StunPacket::checkAuthentication(const std::string &ufrag, const std::string &password) const {
    ...
    switch (_klass) {
        case Class::REQUEST: {
            ...
            if (getMethod() == Method::ALLOCATE || getMethod() == Method::REFRESH || 
                ...
                // TURN authentication: USERNAME should be equal to ufrag
                ...
                // ICE authentication: USERNAME format is local-ufrag:remote-ufrag (only used for ICE BINDING requests)
                std::string username = getUsername();
                if (!username.empty()) {
    ...
    if (attr_message_integrity) {
        ...
        if (attr_nonce && attr_realm) {
            ...
            // According to RFC 5389/5766 standard: key = MD5(username ":" realm ":" password)
    ...
    if (hasAttribute(StunAttribute::Type::FINGERPRINT)) {
        ...
        if (attr_fingerprint) {
            //Calculate FINGERPRINT: Calculate CRC32 on the entire packet except the FINGERPRINT attribute
...
void StunPacket::serialize() {
    ...
    if (message_integrity_size) {
        ...
        //According to RFC 5766 standard: key = MD5(username ":" realm ":" SASLprep(password))
        std::string key = password;
        if (attr_nonce && attr_realm) {
            // Use long-term credentials
            // key = MD5(username ":" realm ":" password)
            auto realm = attr_realm->getRealm();
            std::string username = ufrag; //For response messages, use ufrag as username
...
StunPacket::Ptr StunPacket::createSuccessResponse() const {
    ...
    //Copy authentication-related attributes to the response packet for MESSAGE-INTEGRITY calculation

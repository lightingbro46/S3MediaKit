#ifndef S3MEDIAKIT_WEBRTC_ICE_TRANSPORT_HPP
#define S3MEDIAKIT_WEBRTC_ICE_TRANSPORT_HPP
...
public:
    ...
    enum class State {
        Frozen = 1,         //Not checked yet, and no need to check yet
        Waiting,            //The check has not been sent yet, but it is not frozen either.
        InProgress,         //check has been initiated, but is still in progress
    ...
    // Get the candidate address type string static function
...
//The format is: (stun/turn)[s]:host:port[?transport=(tcp/udp)], default udp mode
//Example:
// stun:stun.l.google.com:19302 → Google's STUN server (UDP).
// turn:turn.example.com:3478?transport=tcp → TURN server using TCP.
// turns:turn.example.com:5349 → TURN server using TLS.
...
public:
    ...
    public:
        ...
        void get_peer_addr(sockaddr_storage &peer_addr) const {
            if (!_peer_host.empty()) {
                ...
                if (addr->sa_family == AF_INET6 && IN6_IS_ADDR_V4MAPPED(&((struct sockaddr_in6 *)addr)->sin6_addr)) {
                    ...
                    // Convert IPv6 v4mapped address to IPv4 address
        ...
        static bool is_same(Pair* a, Pair* b) {
            // FIXME: After a->_socket == b->_socket condition is established, get_peer_ip and get_peer_port must be the same later.
    ...
    public:
        ...
        //Peer end host:port address, because multiple pairs will reuse a socket object, it may be inconsistent with the bind information created by _socket.
        std::string _peer_host;
        uint16_t _peer_port;

        //The post-relay address is used to implement the TURN forwarding address. When the address is not empty, the address is the real peer address. _peer_host and _peer_port represent the relay address.
...
public:
    ...
    struct RequestInfo {
        StunPacket::Ptr _request;       // Original request package
        MsgHandler _handler;            // Response processing function
        Pair::Ptr _pair;                // Send pair
        uint64_t _send_time;            // First send time (milliseconds)
        uint64_t _next_timeout;         // Next timeout (milliseconds)
        uint32_t _retry_count;          // Current number of retransmissions
        uint32_t _rto = 500;            // Current RTO value (milliseconds) Initial RTO 500ms
...
public:
    ...
    //Candidate pair information structure
    struct CandidatePair {
        Pair::Ptr _local_pair;            //local candidate pair
        CandidateInfo _remote_candidate;  //Remote candidate information
        CandidateInfo _local_candidate;   //Local candidate information
        uint64_t _priority;               //Candidate pair priority (64 bits, compliant with RFC 8445)
        CandidateInfo::State _state;      //Connectivity check status
        ...
        // Comparison operator for priority sorting (higher priority first)
    ...
    enum class State {
        ...
        Running = 1,        //Connectivity check of candidate address in progress
        Nominated,          //Initiate nomination and wait for response
        Completed,          //All candidates have been verified, and at least one connection check succeeded
        Failed,             //All candidate checks failed, connection unavailable
    };
    ...
    // Get checklist information for API query
...
protected:
    ...
    // Bidirectional index candidate address management structure
    struct SocketCandidateManager {
        // One-to-many mapping from socket to candidates
        std::unordered_map<toolkit::SocketHelper::Ptr, std::vector<CandidateInfo>> socket_to_candidates;
        
        // Mapping from candidate to socket (for quick lookup)
        ...
        // Socket lists grouped by type for easy traversal
        std::vector<toolkit::SocketHelper::Ptr> _host_sockets;    // HOST type sockets
        std::vector<toolkit::SocketHelper::Ptr> _relay_sockets;   // RELAY type sockets

        bool _has_relayed_candidate = false;

        // Add mapping with 5-tuple duplicate check
        bool addMapping(toolkit::SocketHelper::Ptr socket, const CandidateInfo& candidate) {
            // Check if 5-tuple already exists
            if (candidate_to_socket.find(candidate) != candidate_to_socket.end()) {
                return false; // Same 5-tuple already exists
            }
        ...
        // Get all candidates corresponding to a socket
        ...
        // Get the socket corresponding to a candidate
        ...
        // Get all sockets (for easy traversal)
        ...
        // Get all candidates (for easy traversal)
    ...
    //TODO: Currently only supports multiplexing multiple data streams with one checklist
...
#endif //S3MEDIAKIT_WEBRTC_ICE_TRANSPORT_HPP

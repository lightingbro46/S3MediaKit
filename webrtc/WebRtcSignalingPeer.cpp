...
// Registered signaling server list
// Registering to the same server address is not allowed
...
void addWebrtcRoomKeeper(const string &host, uint16_t port, const std::string& room_id, bool ssl,
    ...
    if (s_room_keepers.find(key)) {
        // Already initiated registration
...
void WebRtcSignalingPeer::onError(const SockException &err) {
    ...
    // Unless the peer explicitly sends a logout execution, the session interruption caused by network abnormality will not affect the webrtc session that has been communicating, and will only be removed.
}
...
void WebRtcSignalingPeer::checkResponseExpired() {
    //FIXME: Moved to a dedicated timeout timer for processing
#if 0
    // Set timer to detect offer response timeout

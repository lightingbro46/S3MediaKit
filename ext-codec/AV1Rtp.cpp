...
// RTP aggregation header bit definitions
...
// LEB128 encoding/decoding helper functions
...
static bool readLeb128(const uint8_t*& data, size_t& remaining, uint64_t& value) {
    ...
    // Compatibility handling: if the end of data is reached but the MSB of the last byte is still 1,
    // assume this is the end of the leb128 encoding
...
bool AV1RtpEncoder::inputFrame(const Frame::Ptr &frame) {
    ...
    // Check if it contains sequence header (keyframe flag)
    ...
    // If no keyframe has been received yet and the current frame is not a keyframe, drop it
...
bool AV1RtpDecoder::inputRtp(const RtpPacket::Ptr &rtp, bool key_pos) {
    ...
    // If starting a new coded video sequence, clear previous state
...
bool AV1RtpDecoder::processPayload(const AggregationHeader& agg_header,
    ...
    while (remaining > 0) {
        ...
        if (has_size) {
            if (!readLeb128(data, remaining, element_size)) {
                ...
                // Compatibility fallback: if leb128 parsing fails, try using the remaining byte count directly
    ...
    if (expected_elements > 0 && static_cast<int>(element_index) != expected_elements) {
        ...
        // Do not return false, continue processing to improve compatibility
    }
...
bool AV1RtpDecoder::emitObu(const uint8_t* data, size_t size) {
    ...
    // Check if the OBU header already contains the size bit
    bool already_has_size = obuHasSize(obu_header);

    // If the OBU in the RTP packet already contains the size field, special handling is required
    if (already_has_size) {
        //WarnL << "RTP OBU contains size field";

        // Skip extension header processing
        ...
        // Directly copy the complete OBU (including the existing size field)
        _frame->_buffer.append((char*)data, size);
    } else {
        // Standard case: OBU in RTP packet does not have size field, we need to add it

        // Write OBU header with size bit set
        ...
        // Calculate payload size and write leb128 encoded size field

...
bool VP9RtpDecoder::decodeRtp(const RtpPacket::Ptr &rtp) {
    ...
    if (!start && seq != (uint16_t)(_last_seq + 1)) {
        // The seq of the middle or last rtp packet must be continuous, otherwise it means rtp packet loss, then the frame is incomplete and must be discarded
        _frame_drop = true;
        _frame->_buffer.clear();
        return false;
    }
    ...
    if (info.endingOfLayerFrame) { // rtp->getHeader()->mark
        // Ensure the next packet must be beginningOfLayerFrame
        _frame_drop = true;

...
// Remove 00 00 03 anti-race byte
...
// Extract profile/level/tier information from VPS or SPS
HevcProfileInfo parse_hevc_profile_tier_level(const uint8_t *nalu, size_t size) {
    // Remove the start code (00 00 01 or 00 00 00 01)
    size_t offset = 0;
    ...
    //Both VPS and SPS include profile_tier_level()
    //Parse the minimum required parts first

    // vps_video_parameter_set_id 或 sps_video_parameter_set_id (skip)
    br.readBits(4);

    // There is also sps_max_sub_layers_minus1 in sps
    ...
    // general_progressive_source_flag etc. (skip)

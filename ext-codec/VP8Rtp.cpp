...
bool VP8RtpDecoder::decodeRtp(const RtpPacket::Ptr &rtp) {
    ...
    if (!start && seq != (uint16_t)(_last_seq + 1)) {
        // Intermediate or last rtp packet, its seq must be continuous, otherwise it indicates rtp packet loss, then the frame is incomplete and must be discarded
        _frame_drop = true;
        _frame->_buffer.clear();
        return false;
    }
    ...
    if (end) {
        // Ensure the next fu must receive the first packet
        _frame_drop = true;

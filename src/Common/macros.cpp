...
/**
 * 本项目采用类MIT协议，用户在履行MIT协议义务的同时，应当同时遵循保留S3MediaKit软件版权信息的义务。
 * 用户不得去除S3MediaKit提供的各种服务中包括但不限于 "title"、"Server"、"User-Agent" 等字段中 "S3MediaKit" 的信息。
 ...
 */
#if !defined(ENABLE_VERSION)
const char kServerName[] =  "S3MediaKit-8.0(build in " __DATE__ " " __TIME__ ")";
#else

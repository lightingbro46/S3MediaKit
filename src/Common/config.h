...
using Seek2Invoker = std::function<void(const uint64_t&, const std::map<std::string, std::map<uint64_t, std::string>> &)>;
// Broadcast for seeking rtsp/rtmp/http-flv events. Control playback seeking through this event.
...
#define BroadcastGetStreamQualityArgs const std::string &vhost, const std::string &device_id, const Broadcast::StreamQualityInvoker &invoker
...
// Broadcast for device statistics changed event.
extern const std::string kBroadcastDeviceStatsChanged;
#define BroadcastDeviceStatsChangedArgs DeviceSource &sender
...
#define BroadcastSyncChangesArgs const std::string &origin_urls, const std::string &peer_id, const std::string &db_guid, const std::vector<TransactionSequence> &since_cursors, const int &batch_limit, const Broadcast::OnResInvoker &invoker
...
extern const std::string kBroadcastSyncBookmarkIndex;
...
extern const std::string kBroadcastSyncBookmarkThumbnail;

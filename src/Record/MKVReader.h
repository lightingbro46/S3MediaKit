#ifndef SRC_MEDIAFILE_MKVREADER_H_
#define SRC_MEDIAFILE_MKVREADER_H_
#ifdef ENABLE_MKV
...
class MKVReader : public std::enable_shared_from_this<MKVReader>, public MediaSourceEvent {
public:
    using Ptr = std::shared_ptr<MKVReader>;

    /**
     * Play an mkv file and convert it to a MediaSource stream
     * @param vhost Virtual host
     * @param app Application name
     * @param stream_id Stream id, if empty, only demultiplex mkv, but not generate MediaSource
     ...
     */
    MKVReader(const MediaTuple &tuple, const std::string &file_path = "", toolkit::EventPoller::Ptr poller = nullptr);

    MKVReader(const MediaTuple &tuple, const std::string &file_path, const ProtocolOption &option, toolkit::EventPoller::Ptr poller = nullptr);

    /**
     * Start demultiplexing the MKV file
     ...
     */
    void startReadMKV(uint64_t sample_ms = 0, bool ref_self = true,  bool file_repeat = false);

    /**
     * Stop demultiplexing the MKV timer
     */
    void stopReadMKV();

    /**
     * Get the mkv demultiplexer
     */
    const MultiMKVDemuxer::Ptr& getDemuxer() const;
...
#endif // ENABLE_MKV
#endif // SRC_MEDIAFILE_MKVREADER_H_
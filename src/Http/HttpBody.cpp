...
static std::shared_ptr<char> getSharedMmap(const string &file_path, int64_t &file_size) {
    ...
    if (hfile == INVALID_HANDLE_VALUE) {
     ...
     GetFileSizeEx(hfile, &FileSize); //The extension of the GetFileSize function can be used to obtain file sizes larger than 4G
...
HttpFileBody::HttpFileBody(const string &file_path, bool use_mmap) {
    ...
    // Determine whether it is a directory to avoid mmap operations on the directory, which may cause the program to crash.

...
API_EXPORT int API_CALL mk_recorder_start_task(const char *vhost, const char *app, const char *stream, const char *path, uint32_t back_ms, uint32_t forward_ms) {
    ...
    if (!src) {
        WarnL << "The relevant MediaSource was not found and startRecordTask failed.:" << vhost << "/" << app << "/" << stream;
        return false;
    }
    bool ret;
    src->getOwnerPoller()->async([=]() mutable {
        ...
        try {
            ...
            err = ex.what();
            WarnL << "MediaSource failed to open startRecordTask:" << vhost << "/" << app << "/" << stream << " what: " << err;
        }
        ret = err.empty();
    });
    return ret;

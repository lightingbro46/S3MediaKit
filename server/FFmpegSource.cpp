...
void FFmpegSnap::makeSnap(const std::string &camera_id, const string &stream_id, uint64_t pos_time, const std::string &save_path, float timeout_sec, const onSnap &cb) {
    GET_CONFIG(string, ffmpeg_bin, FFmpeg::kBin);
    GET_CONFIG(string, ffmpeg_snap, FFmpeg::kSnap);
    GET_CONFIG(string, ffmpeg_log, FFmpeg::kLog);
    Ticker ticker;
    WorkThreadPool::Instance().getPoller()->async([timeout_sec, camera_id, stream_id, pos_time, save_path, cb, ticker]() {
        auto start_time = 0;
        ...
        TimeRecorder::Instance().getRecordedFootage(start_time, end_time, camera_id, stream_id, [&](const vector<TimeBlock> &blocks) mutable {
            ...
            for (const auto &block : blocks) {
                ...
                play_url = block.file_path();
            }
        });
        if (play_url.empty()) {
            // Can not find data
            cb(false, "can not find data");
            return;
        }

...
const string kSnapOverlay = FFmpeg_FIELD"snap_overlay";
...
void FFmpegSnap::makeSnapWithFilter(const string &play_url, const string &save_path,
                                    uint64_t seek_time, float timeout_sec,
                                    const string &filter_complex, const onSnap &cb) {
    GET_CONFIG(string, ffmpeg_bin, FFmpeg::kBin);
    GET_CONFIG(string, ffmpeg_snap_overlay, FFmpeg::kSnapOverlay);
    GET_CONFIG(string, ffmpeg_log, FFmpeg::kLog);
    Ticker ticker;
    WorkThreadPool::Instance().getPoller()->async([timeout_sec, play_url, save_path, seek_time,
                                                    filter_complex, cb, ticker]() {
        auto elapsed_ms = ticker.elapsedTime();
        if (elapsed_ms > timeout_sec * 1000) {
            cb(false, "wait work poller schedule snap task timeout");
            return;
        ...
        process->run(cmd, log_file);
        auto delayTask = EventPollerPool::Instance().getPoller()->doDelayTask(
            static_cast<uint64_t>(timeout_sec * 1000 - elapsed_ms), [process]() {
                if (process->wait(false)) {
                    process->kill(2000);
                }
                return 0;
...
// Build privacy-mask stages from fixed-canvas polygon alpha masks. The asset
// is the first scale2ref input and the source is the reference input, so FFmpeg
// scales each fixed-size SVG asset to the source dimensions at runtime.
std::string FFmpegOverlayFilter::buildPrivacyMask(
    ...
    for (size_t i = 0; i < masks.size(); ++i) {
        ...
        const std::string mask_svg = OverlayPrivacyUtils::buildPrivacyMaskSvg(
            single_mask, canvas_width, canvas_height, mask.mask_type,
            mask.mask_type != PrivacyMaskRegion::SOLID);
        if (!File::create_path(mask_path, 0777) || !File::saveFile(mask_svg, mask_path)) {
            WarnL << "FFmpeg overlay: cannot save polygon privacy mask SVG to " << mask_path;
            success = false;
            last_label = "0:v";
            return "";
        }
        ...
        graph << "movie=" << OverlayPrivacyUtils::escapeMoviePath(mask_path)
              << ",loop=loop=-1:size=1:start=0[" << next << "_asset];";
        if (mask.mask_type == PrivacyMaskRegion::SOLID) {
            graph << "[" << next << "_asset][" << current << "]"
                  << "scale2ref[" << next << "_mask][" << next << "_base];"
            ...
            if (mask.mask_type == PrivacyMaskRegion::BLUR) {
                ...
                graph << "[" << next << "_src]scale=w=\\'max(1,trunc(iw/12))\\':"
                      ...
                      << "scale=w=iw*12:h=ih*12:flags=neighbor[" << next << "_processed];";
            }
...
std::string FFmpegOverlayFilter::build(const MediaTuple &tuple,
    ...
    overlay_required = use_watermark || use_privacy_mask || use_source_stamp;
    ...
    if (use_watermark && use_watermark_asset) {
        ...
        if (!OverlayPrivacyUtils::resolveWatermarkAsset(policy.watermark_template, options,
            ...
            WarnL << "FFmpeg overlay: cannot resolve watermark SVG asset for " << tuple.app
                  << ": " << watermark_error;
            return "";
        }
    } else if (use_watermark) {
        if (!OverlayPrivacyUtils::parseComponents(policy.watermark_template, components, options)) {
            WarnL << "FFmpeg overlay: watermark template is invalid for " << tuple.app;
            return "";
        } else {
            std::string local_image_error;
            if (!OverlayPrivacyUtils::resolveLocalImages(components, local_image_error)) {
                WarnL << "FFmpeg overlay: cannot resolve watermark image assets for " << tuple.app << ": " << local_image_error;
                return "";
            }
        }
    }
    ...
    if (use_privacy_mask && !OverlayPrivacyUtils::parsePrivacyMasks(policy.privacy_mask_regions, privacy_masks, options)) {
        WarnL << "FFmpeg overlay: privacy mask configuration is invalid for " << tuple.app;
        return "";
    }
    if ((use_watermark && components.empty() && watermark_path.empty()) ||
        (use_privacy_mask && privacy_masks.empty())) {
        WarnL << "FFmpeg overlay: required overlay has no renderable content for " << tuple.app;
        return "";
    }

    std::string last_label;
    bool privacy_filter_ok = true;
    std::string filter_complex = buildPrivacyMask(
        privacy_masks, options.canvas_width, options.canvas_height,
        svg_path, temporary_paths, last_label, privacy_filter_ok);
    if (!privacy_filter_ok) {
        return "";
    }
...
static Broadcast::ViewOverlayPolicy resolveViewOverlayPolicy(const MediaTuple &tuple,
    ...
    Broadcast::ViewOverlayPolicyInvoker invoker = [&](const Broadcast::ViewOverlayPolicy &value) {
        policy = value;
    };
...
bool FFmpegOverlayFilter::buildThumbnailOverlay(const MediaTuple &tuple, const string &jwt_token,
                                                const string &path_prefix,
                                                string &filter_complex, vector<string> &temporary_paths,
                                                string &policy_key) {
    const Broadcast::ViewOverlayPolicy policy = resolveViewOverlayPolicy(tuple, jwt_token);

    const bool use_watermark = policy.watermark_enforce && !policy.watermark_excluded;
    const bool use_privacy_mask = policy.privacy_mask_enforce && !policy.privacy_mask_excluded;
    policy_key = StrPrinter << use_watermark << "|" << use_privacy_mask << "|"
                            << policy.watermark_template << "|" << policy.privacy_mask_regions
                            << "|" << policy.username << "|" << policy.camera_name;
    if (!use_watermark && !use_privacy_mask) {
        policy_key = "none";
    ...
    filter_complex = FFmpegOverlayFilter::build(tuple, policy, options, path_prefix,
                                                temporary_paths, overlay_required);
    if (!overlay_required || filter_complex.empty()) {
        WarnL << "Thumbnail overlay could not be rendered for " << tuple.app;
        return false;
    }
    return true;
}
...
void FFmpegExtractor::makeExtract(const string &key, const string &root_path, const onExtract &cb) {
    ...
    std::vector<std::string> temporary_paths;
    const Broadcast::ViewOverlayPolicy policy = resolveViewOverlayPolicy(_tuple, _options.jwt_token);
    bool overlay_required = false;
    auto filter_complex = FFmpegOverlayFilter::build(_tuple, policy, _options, overlay_svg_path,
                                                     temporary_paths, overlay_required);

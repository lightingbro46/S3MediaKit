#ifndef S3MEDIAKIT_SHELLPARSER_H
#define S3MEDIAKIT_SHELLPARSER_H
...
ParseResult parse_shell_like(const std::string &input) {
        ...
        switch (st) {
            case State::Normal: {
                if (is_space(c)) {
                    ...
                    ++i; // consume backslash
                    if (i >= N) {
                        return {false, "An isolated backslash at the end (no characters escaped)", i, {}};
                    }
            ...
            case State::InDouble: {
                if (c == '"') { st = State::Normal; ++i; }
                    ...
                    ++i; // consume backslash
                    if (i >= N) {
                        return {false, "An isolated backslash at the end of double quotes (no characters escaped)", i, {}};
                    }
    ...
    if (st == State::InSingle) {
        return {false, "Missing matching single quote (')", i, {}};
    }
    if (st == State::InDouble) {
        return {false, "Missing matching double quote (\")", i, {}};
    }
...
#endif // S3MEDIAKIT_SHELLPARSER_H
